...
class VecOnlineIQAlgorithm(BaseRLAlgorithm, metaclass=abc.ABCMeta):
    def __init__(
            self,
            trainer,
            exploration_env,
            ...
            for _ in range(self.num_train_loops_per_epoch):
                for _ in range(self.num_expl_steps_per_train_loop // self.expl_env.env_num):
                    ...
                    for _ in range(num_trains_per_expl_step):
                        expert_data = self.expert_buffer.next_batch(self.batch_size)
                        self.trainer.train(train_data, expert_data)
